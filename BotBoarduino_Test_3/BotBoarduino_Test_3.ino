...
//--------------------------------------------------------------------
//[REMOTE]                 
#define cTravelDeadZone         1    //The deadzone before start walking
//====================================================================
//[ANGLES]
...
int vibration_0;
int vibration_3;
int last_vibration_0;
int last_vibration_3;
int counter_reaction;
int state;
int counter_vibration;
int selected_action;
//-----------------------------------------------------------------------------
...
void setup(){
    ...
    g_InControlState.BodyPos.y = OFFSET_Y;
    ...
    g_InControlState.LegLiftHeight = 20;
    GaitStep = 1;
    GaitSelect();
...
//-----------------------------------------------------------------------------
    vibration_0_acc = 0;
    vibration_3_acc = 0;
    last_vibration_0 = 511;
    last_vibration_3 = 511;
    vibration_0 = 511;
    vibration_3 = 511;
    counter_reaction = 20;
    state = 0;
...
void restoreInitialPosition() {
  if (g_InControlState.BodyPos.y > OFFSET_Y) {
    g_InControlState.BodyPos.y -= 1;
  }
  
  if (g_InControlState.TravelLength.z > 0) {
    g_InControlState.TravelLength.z -= 1;
  }
  
  if (g_InControlState.LegLiftHeight > 0) {
    g_InControlState.LegLiftHeight -= 10;
  }  

  if (abs(g_InControlState.BodyRot1.x) > 0) {
    if (g_InControlState.BodyRot1.x > 0) {
      g_InControlState.BodyRot1.x -= 1;
    }
    else{
      g_InControlState.BodyRot1.x += 1;
    }
  ...
  LegPosX[0] = (short)pgm_read_word(&cInitPosX[0]);
  LegPosX[2] = (short)pgm_read_word(&cInitPosX[2]);
  LegPosX[3] = (short)pgm_read_word(&cInitPosX[3]);
  LegPosX[5] = (short)pgm_read_word(&cInitPosX[5]);
  LegPosY[0] = (short)pgm_read_word(&cInitPosY[0]);
  LegPosY[2] = (short)pgm_read_word(&cInitPosY[2]);
  LegPosY[3] = (short)pgm_read_word(&cInitPosY[3]);
  LegPosY[5] = (short)pgm_read_word(&cInitPosY[5]);
}


void agressiveReaction(boolean frontBack) {
  Serial.println("Agressive");
  g_InControlState.SpeedControl = 50;
  g_InControlState.BodyPos.y = 20;
  if (frontBack) {
    g_InControlState.BodyRot1.x = -40;
    LegPosX[0] = 10;
    LegPosX[3] = 10;
    LegPosY[0] = -180;
    LegPosY[3] = -180;
...
void selectAction(int diff1, int diff2, int th1, int th2) {
  if ( (diff1 > th1) || (diff2 > th1) ) {
    restoreInitialPosition();
    if ( (diff1 > th2) || (diff2 > th2) ) {
      agressiveReaction(diff1 > diff2);
      selected_action = 1;
    }
    else {
      switch (random(2)) {
...
void sft() {
  int diff_0 = abs(vibration_0 - last_vibration_0);
  int diff_3 = abs(vibration_3 - last_vibration_3);
  
  switch (state) {
    case 0:                                //set setpoints
      selectAction(diff_0, diff_3, 2, 4);
      break;
    case 1:                                //wait 1
      if (counter_reaction > 0) {
        counter_reaction--;
      }
      else {
        counter_reaction = 20;
    ...
    case 2:                                //wait 2
      if (counter_reaction > 0) {
        ...
        if (selected_action != 1) {
          selectAction(diff_0, diff_3, 3, 3);
        }
      }
      else {
        counter_reaction = 30;
    ...
    case 3:                                //back to initial position
      if (counter_reaction > 0) {
        counter_reaction--;
        restoreInitialPosition();
        selectAction(diff_0, diff_3, 3, 4);
      }
      else {
        Serial.println("End");
        state = 0;
...
void readVibration() {
  vibration_0_acc += analogRead(A0);
  vibration_3_acc += analogRead(A3);
  counter_vibration++;
}
...
void loop(void) {
    ...
    if (counter_vibration < 4) {          // accumulate vibration sensors 4 times (filtering)
      readVibration();
    }
    else {
      last_vibration_0 = vibration_0;
      last_vibration_3 = vibration_3;
      vibration_0 = vibration_0_acc >> 2;
      vibration_3 = vibration_3_acc >> 2;
      vibration_0_acc = 0;
      vibration_3_acc = 0;
      counter_vibration = 0;

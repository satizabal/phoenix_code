...
SerialGamepad ps2x; // create PS2 Controller Class
...
//    g_BodyYOffset = 45;  // 0 - Devon wanted...
    g_BodyYOffset = MIN_HEIGHT;  // 0 - Devon wanted...
    g_BodyYShift = 0;
        ...
        if (g_InControlState.fHexOn) {
            ...
            if (ps2x.ButtonPressed(PSB_L1)) {// L1 Button Test
                ...
                if (ControlMode != TRANSLATEMODE ) {
                    ...
                    ps2x.printMessage("trans");
                }
                else {
                    if (g_InControlState.SelectedLeg==255) {
                        ControlMode = WALKMODE;
                        ps2x.printMessage("walk");
                    }
                    else {
                        ControlMode = SINGLELEGMODE;
                        ps2x.printMessage("single");
                    }
                }
            }
            ...
            //Single leg mode fNO
            if (ps2x.ButtonPressed(PSB_CIRCLE)) {// O - Circle Button Test        3
...
#ifdef OPT_GPPLAYER
            // GP Player Mode X
            if (ps2x.ButtonPressed(PSB_CROSS)) { // X - Cross Button Test          2
                MSound(SOUND_PIN, 1, 50, 2000);  //sound SOUND_PIN, [50\4000]
            ...
            if (ps2x.ButtonPressed(PSB_SQUARE)) { // Square Button Test            1
            ...
            //Stand up, sit down  
            if (ps2x.ButtonPressed(PSB_TRIANGLE)) { // Triangle - Button Test      4
                if (g_BodyYOffset>0) 
                    g_BodyYOffset = 0;
                else
                    g_BodyYOffset = 35;
            ...
            if (ps2x.ButtonPressed(PSB_PAD_UP))// D-Up - Button Test
              if (g_BodyYOffset < MAX_HEIGHT) {
                g_BodyYOffset += 3;
              }
            if (ps2x.ButtonPressed(PSB_PAD_DOWN))// D-Down - Button Test
              if (g_BodyYOffset > MIN_HEIGHT) {
                g_BodyYOffset -= 3;
              }
